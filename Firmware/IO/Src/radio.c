// The intention is to use a radio that supports CRSF https://github.com/tbs-fpv/tbs-crsf-spec/blob/main/crsf.md
// CRSF packets are as follows => 0: address 1: length of data left in the packet/payload (N) 2: type of data 3-N: data N+1: CRC 
// CRSF payload are at most 64 bytes (including CRC)


#include "radio.h"
#include <stdbool.h>
#include <stdint.h>

// CRSF lookup table
static const uint8_t crc8tab[256] = {
    
};

// Current values of the stick controls
static RC_Channels latest_channels = {992, 992, 992, 172};
static bool new_channel_data = false;

// Create the Cyclic Redundancy Check (CRC) from the packet
uint8_t crc8(const uint8_t *ptr, uint8_t len)
{
    uint8_t crc = 0;
    for (uint8_t i=0; i<len; i++)
        crc = crc8tab[crc ^ ptr[i]];
    return crc;
}

// Use a Finite State Machine to process the next byte of data whenever avaliable
void Radio_ProcessByte(uint8_t data){
    static uint8_t state = 0;
    static uint8_t payload[64];
    static uint8_t payload_length = 0;
    static uint8_t payload_index = 0;

    switch (state) {
        case 0: // Looking for device adress
            if (data == 0xC8) state = 1;
            break;
        
        case 1: // Length of data left in payload
            payload_length = data;
            if (payload_length > 2 && payload_length < 64){
                state = 2;
                payload_index = 0;
            } else {
                state = 0;
            }
            break;
        
        case 2: // Type of data
            payload[payload_index++] = data;
            state = 3;
            break;
        
        case 3: // Process payload
            payload[payload_index++] = data;

            if (payload_index == payload_length){
                uint8_t acutal_crc = crc8(payload, payload_length - 1);
                uint8_t recived_crc = payload[payload_length-1];

                // Compare CRCs
                if (acutal_crc == recived_crc){

                    // Check and process for stick command 0x16
                    // Note that stick controlls come in 11-bit little-endian integers (least significant first)
                    // Byte 1 [r8,r7,r6,r5,r4,r3,r2,r1] Byte 2 [p5,p4,p3,p2,p1,r11,r10,r9] Btye 3 [t2,t1,p11,p10,p9,p8,p7,p6] etc
                    if (payload[0] == 0x16){
                        latest_channels.roll = (payload[2] << 8 | payload[1]) & 0x07FF;
                        latest_channels.pitch = (payload[3] << 5 | payload[2] >> 3) & 0x07FF;
                        latest_channels.throttle = (payload[5] << 10 | payload[4] << 2 | payload[3] >> 6) & 0x07FF;
                        latest_channels.yaw = (payload[6] << 7 | payload[5] >> 1) & 0x07FF;

                        new_channel_data = true;
                    }

                    // Add other data types here
                }
                state = 0;
            }
            break;
    }
}

// If there is new data, overwrite the values at the pointer
bool Radio_GetChannels(RC_Channels *data){
    if (new_channel_data) {
        *data = latest_channels;
        new_channel_data = false;
        return true;
    } else {
        return false;
    }
}